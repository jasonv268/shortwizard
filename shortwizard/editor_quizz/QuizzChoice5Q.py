    ...
    def create_sequence(self):
        ...
        for index, item in enumerate(self.json_quizz["questions"]):
            ...
            if index == 4:
                ...
                t += an3.duration
                an3.set_end(t)
                self.sequence.objects += [an1, an3]
                ref_position_reponse = (
                    ref_position_reponse[0], ref_position_reponse[1]+100)

            if "réponse" in item:
                choix: list = item["choix"]
                choix.remove(item["réponse"])
                question = QuestionChoice(
                    item["question"], item["réponse"], random.choice(choix), self.ia_tts).set_start(t)
                t += question.duration
                reponse = Reponse(
